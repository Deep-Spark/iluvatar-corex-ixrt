...
from passes.fusion_layernorm import FusionLayerNormalizationNHWC, FusionLayerNormalization, FusionLayerNormalizationTF
from passes.fusion_groupnorm import FusionGroupNormalization
...
from passes.fusion_gdino_attention import FusionGroundingDinoAttention, FusionGroundingDinoV1_0Attention
...
from passes.replace_CumSum import ReplaceCumSum,ReformatFcBias,FusionSplitQKV
from passes.fusion_MultiscaleDeformableAttn import FusionMultiscaleDeformableAttn 
...
class GroundingDinoOnnxModel(OnnxModel):
    def __init__(self, model: ModelProto, num_heads=16, hidden_size=64):
        """Initialize GroundingDino ONNX Model.
    ...
    def fuse_gdino_attention(self):
        fusion = FusionGroundingDinoAttention(self)
        fusion.apply()

        fusion = FusionGroundingDinoV1_0Attention(self)
        fusion.apply()

    def fuse_group_norm(self):
        fusion = FusionGroupNormalization(self)
        fusion.apply()
    
    def fuse_layer_norm(self):
        fusion = FusionLayerNormalizationNHWC(self)
        fusion.apply()
    ...
    def replace_cumsum(self):
        fusion = ReplaceCumSum(self)
        fusion.apply()
        
    def fuse_MultiscaleDeformableAttn(self):
        fusion = FusionMultiscaleDeformableAttn(self)
        fusion.apply() 
        
    def reformat_fc_bias(self):
        fusion = ReformatFcBias(self)
        fusion.apply() 
        
    
    def fuse_split_qkv(self):
        fusion = FusionSplitQKV(self)
        fusion.apply()     
    ...
    def optimize(
        ...
        self.reformat_fc_bias()
        self.fuse_split_qkv()  
            
        self.fuse_MultiscaleDeformableAttn()    
        self.replace_cumsum()    
        self.fuse_custom_fc()
