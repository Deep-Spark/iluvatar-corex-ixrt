...
emln_plg_creator = plg_registry.get_plugin_creator(
    "CustomEmbLayerNormPluginDynamic_IxRT", "2", ""
)
qkv2_plg_creator = plg_registry.get_plugin_creator(
    "CustomQKVToContextPluginDynamic_IxRT", "3", ""
)
skln_plg_creator = plg_registry.get_plugin_creator(
    "CustomSkipLayerNormPluginDynamic_IxRT", "3", ""
)
...
def custom_fc(prefix, config, init_dict, network, input_tensor, out_dims, W, B):
    ...
    if config.use_int8:
        ...
        if B is not None:
            pf_B = ixrt.PluginField("Bias", B, ixrt.PluginFieldType.FLOAT32)
            amax_vec.append(init_dict[prefix + "out_amax"])
            pf_amax = ixrt.PluginField(
                "fc_amax", np.array(amax_vec, np.float32), ixrt.PluginFieldType.FLOAT32
            )
            fields.append(pf_B)
...
def attention_layer_opt(prefix, config, init_dict, network, input_tensor, imask):
    ...
    pf_dq = ixrt.PluginField(
        "dq_probs", np.array(dq_probs, np.float32), ixrt.PluginFieldType.FLOAT32
    )
    fields.append(pf_dq)
...
def skipln(
    ...
    if is_last_layer:
        pf_fp32 = ixrt.PluginField(
            "output_fp32", np.array([1], np.int32), ixrt.PluginFieldType.INT32
        )
...
def ffn(prefix, config, init_dict, network, input_tensor, residual, is_last_layer):
    ...
    pf_ld = ixrt.PluginField(
        "ld",
        np.array([int(config.intermediate_size)], np.int32),
        ixrt.PluginFieldType.INT32,
    )
    fields = [pf_type, pf_ld]
    if config.use_int8:
        pf_bias = ixrt.PluginField("bias", B_mid, ixrt.PluginFieldType.FLOAT32)
        fields.append(pf_bias)
...
def transformer_layer_opt(
    prefix, config, init_dict, network, input_tensor, imask, residual, is_last_layer
):
    """
    ...
    attention_out_fc = custom_fc(
        prefix + "attention_output_dense_",
        config,
        init_dict,
        network,
        attention_heads,
    ...
    set_output_range(
        attention_out_fc, init_dict[prefix + "attention_output_dense_out_amax"]
    )
    ...
    ffn_layer = ffn(
        ...
        init_dict,
        network,
        skiplayer.get_output(0),
        skiplayer.get_output(1),
        is_last_layer,
    )
    return ffn_layer
...
def bert_model(config, init_dict, network, input_tensor, input_mask, residual):
    ...
    for layer in range(0, config.num_hidden_layers):
        ...
        out_layer = transformer_layer_opt(
            ...
            residual,
            True
            if config.use_int8 and layer == config.num_hidden_layers - 1
            else False,
        )
        ...
        if layer < config.num_hidden_layers - 1:
            set_output_range(
                out_layer,
                init_dict["l{}_".format(layer + 1) + "attention_self_qkv_in_amax"],
            )
        else:
...
def emb_layernorm(
    ...
    output_fp16 = ixrt.PluginField(
        "output_fp16", np.array([1]).astype(np.int32), ixrt.PluginFieldType.INT32
    )
...
def build_engine(batch_sizes, sequence_lengths, config, weights_dict):
    ...
    with builder.create_network(
        ...
        builder_config.set_flag(ixrt.BuilderFlag.INT8)
        ...
        bert_out = bert_model(
            config, weights_dict, network, embeddings, mask_idx, residual_buffer
        )
