...
class BertCalibrator(ixrt.IInt8LegacyCalibrator):
    def __init__(
        self, squad_json, vocab_file, cache_file, batch_size, max_seq_length, num_inputs
    ):
        # Whenever you specify a custom constructor for a IxRT class,
        # you MUST call the constructor of the parent explicitly.
        ixrt.IInt8LegacyCalibrator.__init__(self)
        ...
        self.device_inputs = [
            cuda.mem_alloc(self.max_seq_length * ixrt.int32.itemsize * self.batch_size)
    ...
    # IxRT passes along the names of the engine bindings to the get_batch function.
    # You don't necessarily have to use them, but they can be useful to understand the order of
    # the inputs. The bindings list is expected to have the same ordering as 'names'.
    def get_batch(self, names):
        if self.current_index + self.batch_size > self.num_inputs:
            print(
                "Calibrating index {:} batch size {:} exceed max input limit {:} sentences".format(
        ...
        if current_batch % 10 == 0:
            print(
                "Calibrating batch {:}, containing {:} sentences".format(
