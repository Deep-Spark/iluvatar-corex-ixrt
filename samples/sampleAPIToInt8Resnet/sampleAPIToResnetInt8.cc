...
std::string dir_path("data/resnet18_int8/");
...
nvinfer1::IActivationLayer* basicBlock(nvinfer1::INetworkDefinition* network,
    ...
    // float conv1_input_max = static_cast<const float*>(weight_map[lname + "conv1._input_quantizer._amax"].values)[0];
    // conv1->getInput(0)->setDynamicRange(-conv1_input_max, conv1_input_max);
    float conv1_output_max = static_cast<float const*>(weight_map[lname + "conv1_output_amax_0"].values)[0];
    conv1->getOutput(0)->setDynamicRange(-conv1_output_max, conv1_output_max);
    conv1->setPrecision(nvinfer1::DataType::kINT8);
    layer_num++;

    nvinfer1::IActivationLayer* relu1 = network->addActivation(*conv1->getOutput(0), nvinfer1::ActivationType::kRELU);
    assert(relu1);
    relu1->setPrecision(nvinfer1::DataType::kINT8);
    // float relu1_output_max =
    //     static_cast<float const*>(weight_map["/relu_" + std::to_string(relu_num) + "/Relu_output_amax_0"].values)[0];
    float relu1_output_max =
        static_cast<float const*>(weight_map["Relu_" + std::to_string(layer_num) + "_output_amax_0"].values)[0];
    relu1->getOutput(0)->setDynamicRange(-relu1_output_max, relu1_output_max);
    relu_num++;
    layer_num++;
    ...
    // float conv2_input_max = static_cast<const float*>(weight_map[lname + "conv2._input_quantizer._amax"].values)[0];
    // relu1->getInput(0)->setDynamicRange(-conv2_input_max, conv2_input_max);
    // conv2->getInput(0)->setDynamicRange(-conv2_input_max, conv2_input_max);
    conv2->setPrecision(nvinfer1::DataType::kINT8);
    float conv2_output_max = static_cast<float const*>(weight_map[lname + "conv2_output_amax_0"].values)[0];
    conv2->getOutput(0)->setDynamicRange(-conv2_output_max, conv2_output_max);
    layer_num++;
    ...
    if (inch != outch) {
        ...
        // float conv3_input_max = static_cast<const float*>(weight_map[lname +
        // "downsample.0._input_quantizer._amax"].values)[0]; conv3->getInput(0)->setDynamicRange(-conv3_input_max,
        // conv3_input_max);
        float conv3_output_max = static_cast<float const*>(weight_map[lname + "downsample.0_output_amax_0"].values)[0];
        conv3->getOutput(0)->setDynamicRange(-conv3_output_max, conv3_output_max);
        conv3->setPrecision(nvinfer1::DataType::kINT8);
        layer_num++;
        ew1 = network->addElementWise(*conv3->getOutput(0), *conv2->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
        // float residual_max = static_cast<const float*>(weight_map[lname + "residual_quantizer._amax"].values)[0];
        // ew1->getInput(1)->setDynamicRange(-residual_max, residual_max);
    } else {
        ew1 = network->addElementWise(input, *conv2->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
    }
    ew1->setPrecision(nvinfer1::DataType::kINT8);
    // if (add_num == 0) {
    //     float ew1_output_max = static_cast<float const*>(weight_map["/Add_output_amax_0"].values)[0];
    //     ew1->getOutput(0)->setDynamicRange(-ew1_output_max, ew1_output_max);
    // } else {
    //     float ew1_output_max =
    //         static_cast<float const*>(weight_map["/Add_" + std::to_string(add_num) + "_output_amax_0"].values)[0];
    //     ew1->getOutput(0)->setDynamicRange(-ew1_output_max, ew1_output_max);
    // }
    float ew1_output_max =
        static_cast<float const*>(weight_map["Add_" + std::to_string(layer_num) + "_output_amax_0"].values)[0];
    ew1->getOutput(0)->setDynamicRange(-ew1_output_max, ew1_output_max);
    add_num++;
    layer_num++;

    nvinfer1::IActivationLayer* relu2 = network->addActivation(*ew1->getOutput(0), nvinfer1::ActivationType::kRELU);
    relu2->setPrecision(nvinfer1::DataType::kINT8);
    // float relu2_output_max =
    //     static_cast<float const*>(weight_map["/relu_" + std::to_string(relu_num) + "/Relu_output_amax_0"].values)[0];
    float relu2_output_max =
        static_cast<float const*>(weight_map["Relu_" + std::to_string(layer_num) + "_output_amax_0"].values)[0];
    relu2->getOutput(0)->setDynamicRange(-relu2_output_max, relu2_output_max);
    relu_num++;
    layer_num++;
    assert(relu2);
    return relu2;
...
void ConstructNetwork(nvinfer1::INetworkDefinition* network, const std::string& input_name,
    ...
    conv1->setPrecision(nvinfer1::DataType::kINT8);
    // float conv1_input_max = static_cast<const float*>(weight_map["conv1._input_quantizer._amax"].values)[0];
    float conv1_input_max = static_cast<float const*>(weight_map["input_amax_0"].values)[0];
    conv1->getInput(0)->setDynamicRange(-conv1_input_max, conv1_input_max);
    float conv1_output_max = static_cast<float const*>(weight_map["conv1_output_amax_0"].values)[0];
    conv1->getOutput(0)->setDynamicRange(-conv1_output_max, conv1_output_max);
    layer_num++;
    ...
    assert(relu1);
    relu1->setPrecision(nvinfer1::DataType::kINT8);
    // relu1->getOutput(0)->setDynamicRange(-conv1_output_max, conv1_output_max);
    // float relu1_output_max = static_cast<float const*>(weight_map["/relu/Relu_output_amax_0"].values)[0];
    float relu1_output_max = static_cast<float const*>(weight_map["Relu_1_output_amax_0"].values)[0];
    relu1->getOutput(0)->setDynamicRange(-relu1_output_max, relu1_output_max);
    relu_num++;
    layer_num++;
    ...
    pool1->setPrecision(nvinfer1::DataType::kINT8);
    // float pool1_input_max = static_cast<const float*>(weight_map["maxpool._input_quantizer._amax"].values)[0];
    // pool1->getInput(0)->setDynamicRange(-pool1_input_max, pool1_input_max);
    // float pool1_output_max = static_cast<const float*>(weight_map["/maxpool/MaxPool_output_amax_0"].values)[0];
    float pool1_output_max = static_cast<const float*>(weight_map["MaxPool_2_output_amax_0"].values)[0];
    pool1->getOutput(0)->setDynamicRange(-pool1_output_max, pool1_output_max);
    layer_num++;
    ...
    pool2->setPrecision(nvinfer1::DataType::kINT8);
    // float pool2_input_max = static_cast<const float*>(weight_map["avgpool._input_quantizer._amax"].values)[0];
    // pool2->getInput(0)->setDynamicRange(-pool2_input_max, pool2_input_max);
    // float pool2_output_max =
    //     static_cast<const float*>(weight_map["/avgpool/GlobalAveragePool_output_amax_0"].values)[0];
    float pool2_output_max = static_cast<const float*>(weight_map["GlobalAveragePool_46_output_amax_0"].values)[0];
    pool2->getOutput(0)->setDynamicRange(-pool2_output_max, pool2_output_max);
    layer_num++;

    nvinfer1::IShuffleLayer* flatten = network->addShuffle(*pool2->getOutput(0));
    assert(flatten);
    flatten->setReshapeDimensions(nvinfer1::Dims{2, {1, 512}});
    flatten->setPrecision(nvinfer1::DataType::kINT8);
    flatten->getOutput(0)->setDynamicRange(-pool2_output_max, pool2_output_max);
    layer_num++;

    nvinfer1::IFullyConnectedLayer* fc =
        network->addFullyConnected(*flatten->getOutput(0), 1000, weight_map["fc.weight"], weight_map["fc.bias"]);
    // float fc_output_max = static_cast<const float*>(weight_map["/fc/Gemm_output_amax_0"].values)[0];
    float fc_output_max = static_cast<const float*>(weight_map["Gemm_48_output_amax_0"].values)[0];
    fc->getOutput(0)->setDynamicRange(-fc_output_max, fc_output_max);

    network->markOutput(*fc->getOutput(0));
    PrintDims(fc->getOutput(0)->getDimensions(), "gemm output dim: ");
}

void IxRTAPIConstructModel() {
    std::string image_path(dir_path + "kitten_224.bmp");
    // std::string weight_path(dir_path + "resnet18_fusebn_int8.wts");
    std::string weight_path(dir_path + "resnet18_fusebn_ppq_int8.wts");
    std::string input_name("input");
    std::string output_name("FullyConnected_48_out");
    std::string engine_save_path(dir_path + "api_construct_resnet18_int8.engine");
    ...
    auto cpu_fp32_image = LoadNormalizeImageCPU(image_path, inputDims);
