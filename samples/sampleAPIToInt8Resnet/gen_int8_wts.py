...
if __name__ == "__main__":
    ...
    # net = torchvision.models.quantization.resnet18(weights=torchvision.models.quantization.ResNet18_QuantizedWeights, quantize=True)
    state_dict = torch.load("../../data/resnet18/quant_resnet18-further-calibrated.pth")
    ...
    save_path = join(
        dirname(__file__),
        # "./build/resnet18/resnet18_fusebn_int8.wts",
        "../../data/resnet18/resnet18_fusebn_int8.wts",
    )
    f = open(save_path, "w")
    ...
    for k, v in state_dict.items():
        if k not in fused_net.state_dict().keys():
            if type(v) == torch.Tensor:
                keys_count += 1
    f.write("{}\n".format(keys_count))
    idx = 0
        ...
        if type(v) == torch.Tensor:
            if len(v.size()) > 0:
                ...
                if v.dtype == torch.qint8:  #
                    vr = v.dequantize().detach().reshape(-1).cpu().numpy()
                else:
