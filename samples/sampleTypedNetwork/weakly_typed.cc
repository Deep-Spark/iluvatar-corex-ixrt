...
void WeaklyTypedNetworkSample() {
    ...
    // 弱类型网络可以通过setPrecision接口来实现Layer级别的精度控制
    conv_layer_1->setPrecision(nvinfer1::DataType::kFLOAT);
    ...
    for (int32_t i = 0; i < 16; ++i) {
        if (std::abs(cpu_output.get()[i] - 1.24) > 0.001) {
            result = false;
            break;
        }
    }
    if (result) {
        std::cout << "Infer weakly type finish, pass" << std::endl;
    } else {
        std::cout << "Infer weakly type finish, fail" << std::endl;
    }
}
}  // namespace nvinfer1::samples