...
def extract_nearest_value_by_key_inline(
    content: str, key: str, nearest_distance: int = DEFAULT_NEAREST_MATCH_CHARS
) -> List[float]:
...
def extract_value(
    ...
    if inline:
        if use_re:
            ...
            return extract_nearest_value_by_key_inline(
                content, pattern, nearest_distance
            )
    else:
...
class LogParser(object):
    def __init__(
        ...
        if patterns is None and split_sep is None:
            raise ValueError(
                "The one of argument `patterns` or `split_sep` must be given."
            )

        if pattern_names is not None:
            if (
                isinstance(patterns, (tuple, list))
                and patterns is not None
                and len(patterns) != len(pattern_names)
            ):
                raise ValueError(
                    "The length of `pattern_names` argument not equal to `patterns`."
                )
            if (
                isinstance(split_sep, (tuple, list))
                and split_sep is not None
                and len(split_sep) != len(pattern_names)
            ):
                raise ValueError(
                    "The length of `pattern_names` argument not equal to `split_sep`."
                )

        if split_sep is not None and (
            split_idx is None or not isinstance(split_idx, (int, tuple, list))
        ):
            raise ValueError("Invalid index to split text, got {}.".format(split_idx))

        if split_sep is not None and split_pattern is None:
            raise ValueError(
                "Invalid pattern to split text, got {}.".format(split_pattern)
            )
    ...
    def _parse_inline_by_split(self, line: str, to_type=float):
        ret = {}
        for name, sep, idx, pattern in zip(
            self.pattern_names, self.split_sep, self.split_idx, self.split_pattern
        ):
    ...
    def can_record(self, line: str):
        ...
        if self.start_record:
            if self.end_line_pattern_flag is not None and self.can_matched(
                line, self.end_line_pattern_flag
            ):
